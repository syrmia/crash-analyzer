...
using MemoryValidityMap =
    std::unordered_map<uint64_t, std::pair<uint8_t, uint64_t>>;
...
public:
  ...
  MemoryWrapper();
  Optional<uint64_t> ReadUnsignedFromMemory(uint64_t addr, uint32_t byte_size,
                                            lldb::SBError &error);
  void setDecompiler(Decompiler *Dec);
  void WriteMemory(uint64_t addr, const void *buf, size_t size,
                   lldb::SBError &error);
